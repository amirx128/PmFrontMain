...
export const GetLogisticsQAction = createAsyncThunk(
    ...
    async (
        ...
        try {
            ...
            const {data} = await GetLogisticsQ(userId,0,body.fromDate,body.toDate);
...
export const LogisticsSendItemsAction = createAsyncThunk(
    ...
    async (
        ...
        try {
            const state: any = getState();
            const userId = getUserId(state);
            const {data} = await LogisticsSendItems(userId,0,body.fromDate,body.toDate);
            return fulfillWithValue(data);
        } catch (err) {
            throw rejectWithValue(err);
        }
...
export const GetFinancialQAction = createAsyncThunk(
    ...
    async (
        ...
        try {
            const state: any = getState();
            const userId = getUserId(state);
            const {data} = await GetFinancialQ(userId,0,body.fromDate,body.toDate);
            return fulfillWithValue(data);
        } catch (err) {
            throw rejectWithValue(err);
        }
...
export const FinancialUpdateDetailsActions = createAsyncThunk(
    ...
    async (
        ...
        try {
            const state: any = getState();
            const userId = getUserId(state);
            const {data} = await FinancialUpdateDetails(userId, body.mablaghEtebar, body.purchaseOrderDetailsId);
            return fulfillWithValue(data);
        } catch (err) {
            throw rejectWithValue(err);
        }
...
export const FinancialSendItemsActions = createAsyncThunk(
    ...
    async (
        ...
        try {
            const state: any = getState();
            const userId = getUserId(state);
            const {data} = await FinancialSendItems(userId,0,body.fromDate,body.toDate);
            return fulfillWithValue(data);
        } catch (err) {
            throw rejectWithValue(err);
        }
...
export const GetApproveQAction = createAsyncThunk(
    ...
    async (
        ...
        try {
            const state: any = getState();
            const userId = getUserId(state);
            const {data} = await GetApproveQ(userId,0,body.fromDate,body.toDate);
            return fulfillWithValue(data);
        } catch (err) {
            throw rejectWithValue(err);
        }
...
export const ApproveSendItemsAction = createAsyncThunk(
    ...
    async (
        ...
        try {
            const state: any = getState();
            const userId = getUserId(state);
            const {data} = await ApproveSendItems(userId,0,body.fromDate,body.toDate);
            return fulfillWithValue(data);
        } catch (err) {
            throw rejectWithValue(err);
        }
...
export const ApproveUpdateDetailsAction = createAsyncThunk(
    ...
    async (
        body: { count: number, approveStateId: number, purchaseOrderDetailsId: number },
        {rejectWithValue, fulfillWithValue, dispatch, getState}
    ) => {
        try {
            const state: any = getState();
            const userId = getUserId(state);
            const {data} = await ApproveUpdateDetails(userId, body.count, body.approveStateId, body.purchaseOrderDetailsId);
            return fulfillWithValue(data);
        } catch (err) {
            throw rejectWithValue(err);
        }
...
export const purchaseSlicer = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder
            ...
            .addCase(GetLogisticsQAction.fulfilled, (state: PurchaseState, {payload}) => {
                state.logistics.queue.pending = false;
                state.logistics.queue.data = [...payload?.model];
            })
            .addCase(GetLogisticsQAction.rejected, (state: PurchaseState, {error}) => {
                state.logistics.queue.pending = false;
            });
        //#endregion
        ...
        builder
            ...
            .addCase(LogisticsSendItemsAction.fulfilled, (state: PurchaseState, {payload}) => {
                state.logistics.sendItems.pending = false;
                state.logistics.sendItems.data = [...payload?.model];
            })
            .addCase(LogisticsSendItemsAction.rejected, (state: PurchaseState, {error}) => {
                state.logistics.sendItems.pending = false;
            });
        //#endregion
        ...
        builder
            ...
            .addCase(GetFinancialQAction.fulfilled, (state: PurchaseState, {payload}) => {
                state.financials.queue.pending = false;
                state.financials.queue.data = [...payload?.model];
            })
            .addCase(GetFinancialQAction.rejected, (state: PurchaseState, {error}) => {
                state.financials.queue.pending = false;
            });
        //#endregion
        ...
        builder
            ...
            .addCase(FinancialSendItemsActions.fulfilled, (state: PurchaseState, {payload}) => {
                state.financials.sendItems.pending = false;
                state.financials.sendItems.data = [...payload?.model];
            })
            .addCase(FinancialSendItemsActions.rejected, (state: PurchaseState, {error}) => {
                state.financials.sendItems.pending = false;
            });
        //#endregion
        ...
        builder
            ...
            .addCase(GetApproveQAction.fulfilled, (state: PurchaseState, {payload}) => {
                state.approve.queue.pending = false;
                state.approve.queue.data = [...payload?.model];
            })
            .addCase(GetApproveQAction.rejected, (state: PurchaseState, {error}) => {
                state.approve.queue.pending = false;
            });
        //#endregion
        ...
        builder
            ...
            .addCase(ApproveSendItemsAction.fulfilled, (state: PurchaseState, {payload}) => {
                state.approve.sendItems.pending = false;
                state.approve.sendItems.data = [...payload?.model];
            })
            .addCase(ApproveSendItemsAction.rejected, (state: PurchaseState, {error}) => {
                state.approve.sendItems.pending = false;
            });
        //#endregion
    }
