...
export const GetUsersListAction = createAsyncThunk(
  "administrations/GetUsersListAction",
  async (_, { rejectWithValue, fulfillWithValue, dispatch, getState }) => {
    try {
      const state: any = getState();
      const userId = getUserId(state);
      const { data } = await GetUsersList(userId);
      return fulfillWithValue(data);
    } catch (err) {
      throw rejectWithValue(err);
    }
...
export const administrationSlicer = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    // #region GetUserInfo-----
    builder
      .addCase(GetUsersListAction.pending, (state: AdministrationState) => {
        state.users.usersList = null;
      })
      .addCase(
        GetUsersListAction.fulfilled,
        (state: AdministrationState, { payload }) => {
          state.users.usersList = [...payload?.model];
        }
      )
      .addCase(
