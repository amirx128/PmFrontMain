...
const ExitWarehouseQList = () => {
  ...
  const {
    ...
    handleSelectAll,
  } = useCustomCol("WAREHOUSE_EXIT_WAREHOUSE_QUEUE", warehouseGrid);
  useEffect(() => {
    getList();
  }, []);
  ...
  const getList = async () => {
    ...
    dispatch(GetExitWarehouseQAction(body));
  };
  ...
  return (
    ...
    >
      <Card sx={{ borderRadius: 3 }}>
        ...
        {columns && !exitWarehouseQ.pending && (
          <>
            <Grid
              onDoubleClick={handleDoubleClick}
              columns={columns}
              rows={
                exitWarehouseQ?.data.map((row, index) => ({
                  id: index,
                  ...row,
                })) ?? []
              }
...
export default ExitWarehouseQList;
