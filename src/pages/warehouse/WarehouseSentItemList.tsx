...
const WarehouseSentItemList = () => {
  ...
  const {
    ...
    handleSelectAll,
  } = useCustomCol("WAREHOUSE_WAREHOUSE_SENT_ITEM", warehouseGrid);
  useEffect(() => {
    getList();
  }, []);
  ...
  return (
    ...
    >
      <Card sx={{ borderRadius: 3 }}>
        ...
        {columns && !warehouseSentItem.pending && (
          <>
            <Grid
              onDoubleClick={handleDoubleClick}
              columns={columns}
              rows={
                warehouseSentItem?.data.map((row, index) => ({
                  id: index,
                  ...row,
                })) ?? []
              }
...
export default WarehouseSentItemList;
