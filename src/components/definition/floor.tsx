...
export const FloorCard = ({floor,setSelectedFloor,setAddFloorDialog}) => {
    return (
        <Grid item md={4} sm={6} xs={12}>
            <Box sx={{
                ...
                <Box display={"flex"} justifyContent={"space-between"} alignItems={"center"}>
                    ...
                    <Typography variant={"subtitle1"}>{moment(floor?.createDate).locale('fa').format('LL')}</Typography>
                </Box>
                <Box display={"flex"} gap={0.5} alignItems={"center"} mt={1}>
                    <Box display={"flex"} gap={0.5} alignItems={"center"}>
                        <Book color={"secondary"} sx={{fontSize: 20}} />
                        <Typography color={"secondary"} variant={"subtitle2"}>پروژه: </Typography>
                    </Box>
                    <Box>
