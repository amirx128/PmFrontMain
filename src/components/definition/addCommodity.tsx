...
export const AddCommodity = ({ addCommodityDialog, onClose }) => {
  ...
  const { businessRoles, units, floors, projects, commodities } = useSelector(
    ...
    if (e.target?.name === "useInProjectsIds") {
      dispatch(
        getAllUnits({
          ...
          projectId: info.useInProjectsIds[0],
        })
      );
    }
    ...
    >
      ...
      <DialogContent>
        ...
        <FormControl fullWidth={true}>
          ...
          <Select
            multiple
            value={info?.useInProjectsIds}
            fullWidth={true}
            name={"useInProjectsIds"}
            label={"پروژه مرتبط"}
            onChange={handleChange}
          >
        ...
        <FormControl fullWidth={true}>
          ...
          <Select
            disabled={info?.useInProjectsIds?.length < 1}
            multiple
            value={info?.useInProjectsUnitsIds}
            fullWidth={true}
            name={"useInProjectsUnitsIds"}
            label={"واحد مرتبط"}
            onChange={handleChange}
          >
            {units?.data?.map((item) => (
        ...
        <FormControl fullWidth={true}>
          ...
          <Select
            disabled={info?.useInProjectsUnitsIds?.length < 1}
            multiple
            value={info?.useInProjectsFloorIds}
            fullWidth={true}
            name={"useInProjectsFloorIds"}
            label={"طبقه مرتبط"}
            onChange={handleChange}
          >
